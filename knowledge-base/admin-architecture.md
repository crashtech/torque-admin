...
6. Public `Resource` API: `name`, `controllers` (class names, filled by `Application#setup_controller`; `controller_class` is the first), `sections`, `widgets`, `actions`, `resource_class`, `singular_key`/`singular_title`, `plural_key`/`plural_title`, `inspect`; `resource_name`, `add_resource_path(action, controller, nesting)` (called by the mapper per drawn route), `parent_reflections` (`Resource::ActiveModel`), `member_url_options(record, action: :show)` — the backward lookup (`projects/05-resource-paths.md`). Controller lookups by name go through the application-level memo `Application#controller_class(name)` (`'admin/tasks'`, `'admin/tasks_controller'`, `:tasks` → `Admin::TasksController`), cleared in `Application#clear` on reload — `Resource` and `BaseController#initialized_side_controllers` use it.
